...
import org.gemseeker.sms.views.AddPurchasePaymentWindow;
...
public class PurchasesViewController {
    ...
    private AddPurchasePaymentWindow addPurchasePaymentWindow;
    private PrintWindow printWindow;

    public PurchasesViewController(MainWindow mainWindow, InventoryPanel inventoryPanel, Database database,
    ...
    public void refresh() {
        ...
        disposables.add(Single.fromCallable(() -> paymentController.getPayments(Payment.TYPE_PURCHASE))
                .observeOn(JavaFxScheduler.platform()).subscribeOn(Schedulers.io()).subscribe(list -> {
                    ...
                    hideProgress();
                    mainWindow.showErrorDialog("Database Error", "Error while retrieving purchase payments.\n" + err);
                }));
    }

    private void addItem() {
        if (addPurchasePaymentWindow == null) addPurchasePaymentWindow = new AddPurchasePaymentWindow(this, database, mainWindow.getStage());
        addPurchasePaymentWindow.showAndWait();
        refresh();
    }
    ...
    private void deleteItem() {
        ...
        if (result.isPresent() && result.get() == ButtonType.YES) deletePurchase(selectedItem.get().getId());
    }
